...
> **Status: implementado e validado localmente (2026-06-12).** Migration `0000_clean_spyke.sql` (4 tabelas) aplicada no D1 local; 13 testes unitários + roteiro de aceite `apps/worker/scripts/acceptance-phase1.ts` (22 checks) todos verdes contra `wrangler dev` — isolamento A/B, categorização, summary, cashflow e escopo comprovados.
> Decisões: (a) **sem v1 no repo** — `parser.ts`/`format.ts` não existiam; categorização e keywords foram escritas do zero a partir das regras 6.1 (12 categorias-seed, fallback `outros`/`renda` com flag `isSystem`); (b) `accountId`/`cardId`/`invoiceMonth` já existem como colunas nuláveis, mas FK e o CHECK "exatamente um (conta|cartão)" ficam para as Fases 3/4 (em Fase 1/2 toda transação tem ambos null — o CHECK alvo quebraria isso); (c) auth em dois middlewares: `bearerOnly` (bootstrap/admin) e `internalAuth` (bearer + `X-Telegram-Id` → resolve usuário; nunca confia em userId do cliente); (d) rota interna `POST/GET /api/internal/users` (bearer-only) para bootstrap/admin/testes — o cadastro real por convite chega na Fase 2 e reusa `createUserWithSeed`; (e) datas "hoje"/mês resolvidas no fuso de São Paulo via `Intl`; (f) `/desfazer` já tem suporte no core (`lastTransactionBySource`).
...
> **Status: implementado e validado localmente (2026-06-12).** `apps/worker/scripts/acceptance-phase2.ts` dirige o webhook com updates simulados (modo mock) — 15 checks verdes: bootstrap admin, `/convite`, cadastro por convite, ignorar não-cadastrados, lançamento por texto, categorização, `/resumo`, isolamento A/B via API e `/desfazer`. Falta só rodar o `setWebhook` real (seção 13) — exige o BOT_TOKEN do BotFather.
> Decisões: (a) **bot chama o `core/` direto, em processo** (não self-HTTP) — regra de negócio fica no core e tanto rotas Hono quanto handlers do bot são finos (convenção 14); o "via API com bearer" da seção 8 é honrado pela mesma camada compartilhada; (b) `new Bot` por request com `botInfo` cacheado em módulo (zero `getMe` por update); para teste local sem token, `MOCK_TELEGRAM=1` injeta `botInfo` stub, intercepta as chamadas ao Telegram via transformer e devolve as mensagens no corpo da resposta do webhook; (c) secret do webhook validado manualmente (à prova de versão da grammY), fora do bearer; (d) **bootstrap do admin**: primeiro `/start` vindo de `ADMIN_TELEGRAM_ID` cria o admin sem convite (resolve o ovo-galinha do primeiro usuário); (e) parser escrito do zero — `+`/keyword ⇒ entrada, `-` força saída e vence keyword, prioridade do número com vírgula; comandos extras já incluídos: `/convite`, `/usuarios`.
...
> **Status: implementado e validado localmente (2026-06-12).** Migration `0001` (tabela accounts). `acceptance-phase3.ts` (10 checks) verde: saldo derivado = inicial + entradas pagas − saídas pagas, pendente (`paid:false`) não conta, conta padrão Carteira para lançamento sem indicação, isolamento e escopo (A não lança na conta do B → 422).
> Decisões: (a) **saldo nunca armazenado** — derivado por query agregada (entradas−saídas pagas por conta); (b) seed agora cria a conta "Carteira" (type carteira), que é a conta padrão de lançamentos sem `accountId`/`cardId`; (c) `accountId`/`cardId` seguem como colunas nuláveis **sem FK/CHECK no banco** — integridade garantida no `core/` (validação de posse + invariante "exatamente um"); motivo: SQLite não faz ALTER ADD FK/CHECK sem rebuild da tabela, que brigaria com as migrations incrementais do drizzle-kit e rejeitaria linhas pré-existentes; (d) DELETE de conta = arquivar (`archived`), nunca apaga transações.
...
> **Status: implementado e validado localmente (2026-06-12).** Migration `0002` (tabela credit_cards). `acceptance-phase4.ts` (16 checks) verde: `computeInvoiceMonth` (dia > closingDay ⇒ mês seguinte), gasto no cartão nasce `paid:false` e com `accountId` null (não toca saldo de conta), `GET /invoice` soma a fatura, `pay-invoice` cria saída na `payFromAccountId` (categoria "fatura cartão") + quita os itens em batch atômico, segundo pagamento → 409, invariante conta⊕cartão (ambos → 422), e a sintaxe "mercado 80 no itau" no bot cai no cartão certo.
> Decisões: (a) `createTransaction` centraliza a regra: `cardId` ⇒ calcula `invoiceMonth` (se não vier explícito) e `paid:false` por padrão; conta e cartão são mutuamente exclusivos; (b) total da fatura = Σ saídas − Σ entradas (estornos) daquela `invoiceMonth`; (c) pagamento usa `db.batch([insert, update])` para atomicidade no D1; sem `payFromAccountId` → 409 `no_pay_account`; (d) match de cartão no bot é fuzzy por nome normalizado (acento-insensível), removendo "no/na <cartão>" da descrição; (e) DELETE de cartão = arquivar.
...
> **Status: implementado e validado localmente (2026-06-12).** Migration `0003` (tabela budgets). `acceptance-phase5.ts` (12 checks) verde: `POST /api/transactions` de saída devolve `{ spentCents, limitCents, ratio }` (entrada → null); bot anexa ⚠️ em 80% e 🚨 em 100%; `/orcamento` lista o status do mês; `/api/alerts/preview` retorna os três tipos (pendência vencendo hoje, fatura fechando em 2 dias, orçamento estourado hoje); isolamento. Cron habilitado no `wrangler.toml` (`0 11 * * *` = 08:00 BRT) — só dispara em produção; a lógica foi validada via preview. Falta apenas rodar em produção (precisa do BOT_TOKEN real p/ o sendMessage do cron).
> Decisões: (a) `budget` é upsert por categoria (`unique(userId, categoryId)`); status calculado pelo gasto (saídas) no mês da data da transação; (b) "estouro desde ontem" detectado comparando gasto-até-ontem (`date < hoje`) < limite ≤ gasto-agora — evita repetir o alerta diariamente; (c) cron usa `fetch` direto à API do Telegram (sem grammY); silêncio quando não há alertas; (d) adicionado `GET /api/categories` (o CRUD completo + editor de keywords fica para a Fase 6).
...
> **Status: implementado e validado localmente (2026-06-12).** App `apps/web` + tokens HMAC em `packages/shared` (5 testes). `acceptance-phase6.ts` (8 checks, worker+nuxt reais) verde: magic link → cookie de sessão (30d) → proxy → isolamento A/B, proxy sem cookie → 401, magic link inválido → /login, SSR da home com sessão e redirect sem sessão. `nuxt build` de produção OK. Falta só o deploy na Vercel (conta do usuário) + setar `DASHBOARD_URL` no Worker.
> Decisões: (a) **Nuxt 3.12.4 (pinado)** em vez de Nuxt 4 — as versões 3.14+/4.x usam `oxc-walker`, cujo parser nativo (`oxc-parser`) não instala de forma estável no Windows via bun (binding ausente); 3.12 usa parser acorn e sobe limpo. `bunfig.toml` com `linker = "hoisted"`; (b) auth do Worker ganhou o **modo (b)**: `internalAuth` aceita bearer == API_TOKEN (interno) OU token de sessão HMAC (dashboard); tokens assinados com SESSION_SECRET compartilhado worker↔Nitro via Web Crypto; (c) `/login` no bot gera magic link (10 min) → `${DASHBOARD_URL}/auth?token=`; o server route `/auth` do Nuxt troca por cookie httpOnly e o proxy `server/api/[...path].ts` repassa como Authorization (browser nunca fala com o Worker direto); (d) telas: visão geral (pizza/linha/barras via vue-echarts em ClientOnly), transações (filtros/CRUD/CSV), categorias (editor de keywords), cartões (fatura/pagar), orçamentos, metas; `GET /api/me` adicionado no Worker.
